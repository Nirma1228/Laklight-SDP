...
function Home() {
  ...
  useEffect(() => {
    if (location.hash) {
      // Create a slight delay so the DOM is fully rendered
      setTimeout(() => {
  ...
  return (
    <div className="home-container">
      ...
      {selectedImage && (
        <div 
          className="fixed inset-0 z-[9999] bg-black/90 flex items-center justify-center p-4 backdrop-blur-sm cursor-zoom-out animate-fadeIn"
          onClick={() => setSelectedImage(null)}
        >
          <button 
            className="absolute top-6 right-6 text-white text-3xl hover:text-green-400 transition-colors bg-white/10 w-12 h-12 rounded-full flex items-center justify-center backdrop-blur-md"
            onClick={() => setSelectedImage(null)}
          >
