...
exports.getAllUsers = async (req, res) => {
  try {
    const [users] = await db.query(`
      SELECT u.id, u.full_name, u.email, u.phone, r.role_name as user_type, s.status_name as status, u.join_date 
      FROM users u
...
exports.changeUserStatus = async (req, res) => {
  try {
    const statusId = await getId('account_statuses', 'status_name', req.body.status);
    if (!statusId) return res.status(400).json({ message: 'Invalid status' });
    await db.query('UPDATE users SET status_id = ? WHERE id = ?', [statusId, req.params.id]);
...
exports.getAnalyticsDashboard = async (req, res) => {
  try {
    ...
    const [recentOrders] = await db.query(`
      SELECT o.*, u.full_name as customer_name, os.status_name as status
      FROM orders o
...
exports.createUser = async (req, res) => {
  try {
    ...
    const roleId = await getId('user_roles', 'role_name', role);
    const activeStatusId = await getId('account_statuses', 'status_name', 'active');
...
exports.updateUser = async (req, res) => {
  try {
    ...
    await db.query('UPDATE users SET full_name = ?, phone = ? WHERE id = ?', [fullName, phone, req.params.id]);
...
exports.changeUserRole = async (req, res) => {
  try {
    const roleId = await getId('user_roles', 'role_name', req.body.role);
...
exports.getCustomerAnalytics = async (req, res) => {
  try {
    const [customers] = await db.query('SELECT u.full_name, COUNT(o.id) as order_count, SUM(o.net_amount) as total_spent FROM users u JOIN orders o ON u.id = o.customer_id GROUP BY u.id ORDER BY total_spent DESC');
...
exports.getProductAnalytics = async (req, res) => {
  try {
    const [products] = await db.query('SELECT p.name, COUNT(oi.id) as sales_count, SUM(oi.subtotal) as total_revenue FROM products p JOIN order_items oi ON p.id = oi.product_id GROUP BY p.id ORDER BY total_revenue DESC');
...
exports.getAllFeedback = async (req, res) => {
  try {
    const [rows] = await db.query('SELECT f.*, u.full_name as customer_name FROM feedback f JOIN users u ON f.customer_id = u.id ORDER BY f.created_at DESC');
...
exports.getFeedbackDetails = async (req, res) => {
  try {
    const [rows] = await db.query('SELECT f.*, u.full_name as customer_name, o.order_number FROM feedback f JOIN users u ON f.customer_id = u.id JOIN orders o ON f.order_id = o.id WHERE f.id = ?', [req.params.id]);
